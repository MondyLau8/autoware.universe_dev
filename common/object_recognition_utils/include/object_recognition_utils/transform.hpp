  ...
  T & output_msg)
{
  ...
  if (input_msg.header.frame_id != target_frame_id) {
    output_msg.header.frame_id = target_frame_id;
    tf2::Transform tf_target2clusters_world;
    tf2::Transform tf_target2clusters;
    tf2::Transform tf_clusters_world2clusters;
    {
      const auto ros_target2clusters_world = detail::getTransform(
        tf_buffer, input_msg.header.frame_id, target_frame_id, input_msg.header.stamp);
      if (!ros_target2clusters_world) {
        return false;
      }
      tf2::fromMsg(*ros_target2clusters_world, tf_target2clusters_world);
    }
    for (auto & cluster : output_msg.feature_objects) {
      tf2::fromMsg(cluster.object.kinematics.pose_with_covariance.pose, tf_clusters_world2clusters);
      tf_target2clusters = tf_target2clusters_world * tf_clusters_world2clusters;
      tf2::toMsg(tf_target2clusters, cluster.object.kinematics.pose_with_covariance.pose);
      // TODO(yukkysaito) transform covariance
    }
  }
  return true;
